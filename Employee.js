...
.then(
 ...
 {    
  ...
  dbo.collection("employee").insertOne(myobj)  
    .then(function(){
...
.then(
 ...
 {    
  var dbo = db.db("emp");        
          ...
          var newvalues = { $set: {empid: empid, name:name,dept:dept, salary:salary,designation:designation,mobile:mobile}};
    ...
    var MongoClient = require('mongodb').MongoClient;
        ...
        .then(
         function(db)
         {
            ...
            dbo.collection("employee").find(query).toArray()
            .then(function(result){
               console.log("Fetching emlployee")
             //  res.send(result[0].name);
               console.log(result[0].name);
               res.send("<h1> Name of the emp id "+empid+" is "+result[0].name+"</h1>");
            })
            .catch(function(err){
                  console.log(err)
            })
    ...
    var MongoClient = require('mongodb').MongoClient;
        ...
        .then(
         function(db)
         {
            ...
            var dbo=db.db('emp')            
            var query={empid:empid}
            dbo.collection("employee").deleteOne(query)
            .then(function(result){
